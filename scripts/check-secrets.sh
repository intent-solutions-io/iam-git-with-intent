...
# Files to skip for high-entropy base64 patterns (false positives from integrity hashes, documentation, SHA-pinned actions, and code with long URLs)
SKIP_BASE64_FILES="package-lock.json|pnpm-lock.yaml|yarn.lock|README.md|CLAUDE.md|openapi.yaml|\.test\.ts$|\.spec\.ts$|test/.*\.ts$|examples/.*\.ts$|apps/.*/src/.*\.ts$|\.github/workflows/.*\.yml$"
